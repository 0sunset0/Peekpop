...
- **현재 대상** (`docs/superpowers/plans/2026-08-29-peekpop-mvp.md` Task 2~6과 1:1 대응):
  ...
  - `SubjectSegmenter` — 검증된 크롭 좌표로 (a) 정면 샘플, (b) 기울어진 샘플 각각 마스크 생성 + 그럴싸함 판정, 2케이스
  ...
  - `CreationFlowViewModel` — 화면 전이 로직: 온보딩 분기, 사진 선택 후 전이, 선택된 사진 없이 확정 시 에러 전이, 에러/다시 만들기 시 피커 자동 표시, 5케이스
